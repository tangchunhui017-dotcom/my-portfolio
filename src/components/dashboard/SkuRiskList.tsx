...
type StatusType = '待处理' | '进行中' | '已完成' | '已搞置';
const STATUS_OPTIONS: StatusType[] = ['待处理', '进行中', '已完成', '已搞置'];
const STATUS_STYLE: Record<StatusType, string> = {
    ...
    '已搞置': 'bg-slate-100 text-slate-500 border-slate-200',
};
...
export default function SkuRiskList({ filterSummary = '全部数据' }: SkuRiskListProps) {
    ...
    const exportCsv = () => {
        ...
        const headers = ['款号', '商品名', '品类', '生命周期', '吊牌价', '季节', '进货量', '销量', '净销售额', '售羄率', '折扣深度', '毛利率', '剩余库存', '风险标签', '优先级', '建议动作', '预估收益', '状态'];
        ...
        a.href = url;
        a.download = `SKU风险列表_${filterSummary}_${now.replace(/[/:\s]/g, '-')}.csv`;
        a.click();
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-100">
                            ...
                            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 whitespace-nowrap">状态</th>
                        </tr>
                    </thead>
                        ...
                        {(expanded ? filtered : filtered.slice(0, COLLAPSED_COUNT)).map(row => (
                            <tr key={row.sku_id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-4 py-3 text-center">
                                    <span className={`text-xs px-2 py-1 rounded-md font-bold ${row.priority === 'P0' ? 'bg-red-100 text-red-700' :
                                ...
                                <td className="px-4 py-3 text-center">
                                    <select
                                        ...
                                        className={`text-xs font-semibold px-2 py-0.5 rounded-full border cursor-pointer focus:outline-none ${STATUS_STYLE[statusMap[row.sku_id] ?? '待处理']}`}
                                    >

...
import {
    CHART_TEXT_FAINT, CHART_LINE, CHART_LINE_DASHED,
    CHART_DANGER, CHART_HIGHLIGHT,
...
export default function ChannelInventoryPanel({ filters, compareMode }: Props) {
  ...
  return (
    <ChartCard
      title="渠道库存分布不均衡度"
      subtitle="各渠道类型库存占比与销售占比对比。正值（红色）表示库存积压风险；负值（蓝色）表示断货风险。"
      showMenu={false}
      footer={
        ...
        {!mounted ? <ChartPlaceholder /> : (
          <ReactECharts option={{
            ...
            xAxis: {
              ...
              axisLine: { show: false },
              axisTick: { show: false },
              axisLabel: { color: CHART_TEXT_FAINT, fontSize: 12 },
            },
            yAxis: {
              type: 'value',
              splitLine: { lineStyle: { type: 'dashed', color: CHART_LINE_DASHED } },
              axisLabel: { color: CHART_TEXT_FAINT, fontSize: 11, formatter: (v: number) => `${(v * 100).toFixed(0)}%` },
            },
            series: [{
              name: '不均衡分差',
              type: 'bar',
              ...
              data: channelImbalanceRows.map((r) => ({
                ...
                itemStyle: {
                  color: r.imbalanceScore > 0
                    ? new echarts.graphic.LinearGradient(0, 1, 0, 0, [{ offset: 0, color: '#FCA5A5' }, { offset: 1, color: CHART_DANGER }])
                    : new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: '#93C5FD' }, { offset: 1, color: CHART_HIGHLIGHT }]),
              ...
              markLine: {
                symbol: 'none',
                label: { show: false },
                data: [{ yAxis: 0, lineStyle: { color: CHART_LINE, type: 'solid', width: 1 } }],
              },
            }],

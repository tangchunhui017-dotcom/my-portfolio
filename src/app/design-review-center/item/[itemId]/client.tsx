...
export default function ItemDetailClient({
  ...
  const reviewConfig = REVIEW_STATUS_MAP[item.reviewStatus ?? 'pending'] ?? REVIEW_STATUS_MAP.pending;
  const riskConfig = RISK_LEVEL_MAP[item.riskLevel ?? series.riskLevel] ?? RISK_LEVEL_MAP.medium;
  const reviewConclusion = reviewRecord ? reviewConclusionMap[reviewRecord.conclusion] : null;
  const reviewDimensions = reviewRecord?.dimensions
    ? Object.entries(reviewRecord.dimensions).filter(([, value]) => value)
    : [];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50">
      <div className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <nav className="mb-6 text-sm text-slate-500">
          ...
          <Link href={`/design-review-center/series/${series.seriesId}`} className="hover:text-slate-700">
            {series.seriesName}
          </Link>
          <span className="mx-2">/</span>
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm text-slate-600">
                <div>
                  <div className="text-xs text-slate-400">系列</div>
                  <div className="mt-1 font-medium text-slate-900">{series.seriesName}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">波段</div>
                  <div className="mt-1 font-medium text-slate-900">{item.waveId ?? series.waveId}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">品类</div>
                  <div className="mt-1 font-medium text-slate-900">{item.category}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">使用场景</div>
                  <div className="mt-1 font-medium text-slate-900">{item.occasion}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">产品角色</div>
                  <div className="mt-1 font-medium text-slate-900">{item.productRole}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">设计师</div>
                  <div className="mt-1 font-medium text-slate-900">{item.designer}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">材料</div>
                  <div className="mt-1 font-medium text-slate-900">{item.material}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">配色</div>
                  <div className="mt-1 font-medium text-slate-900">{item.colorway}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">目标上市</div>
                  <div className="mt-1 font-medium text-slate-900">{formatDate(item.targetLaunchDate)}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">下次评审</div>
                  <div className="mt-1 font-medium text-slate-900">{formatDate(item.nextReviewDate)}</div>
                </div>
              </div>
            </section>
              ...
              <div className="mb-4 flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    当前按鞋类单款模板驱动字段展示，后续可继续按品类、品牌线或接飞书字段模板扩展。
                  </p>
                </div>
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">模板驱动</span>
              </div>
                      ...
                      {fields.map((field) => {
                        ...
                        return (
                          <div key={`${group}-${field.key}`} className="rounded-2xl bg-slate-50 p-4">
                            <div className="text-xs font-medium tracking-wide text-slate-400">{field.label}</div>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-xl font-semibold text-slate-800">关联资产</h2>
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">{relatedAssets.length} 项</span>
              </div>
              {relatedAssets.length > 0 ? (
                ...
                <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">当前还没有挂载到该单款的关联资产。</div>
              )}
            </section>
          </div>
          ...
          <div className="space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className={`rounded-full px-3 py-1 text-xs font-medium ${reviewConfig.bgColor} ${reviewConfig.textColor}`}>
                  {reviewConfig.label}
                </span>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl bg-slate-50 p-4">
                  <div className="text-xs font-medium tracking-wide text-slate-400">目标吊牌价</div>
                  <div className="mt-2 text-lg font-semibold text-slate-900">¥ {item.pricePoint}</div>
                </div>
                <div className="rounded-2xl bg-slate-50 p-4">
                  <div className="text-xs font-medium tracking-wide text-slate-400">成本锚点</div>
                  <div className="mt-2 text-sm text-slate-700">目标 {item.targetCostEstimate != null ? `¥ ${item.targetCostEstimate}` : '待补充'}</div>
                  <div className="mt-1 text-xs text-slate-500">锁价 {item.finalLockedCost != null ? `¥ ${item.finalLockedCost}` : '待锁定'}</div>
                </div>
                <div className="rounded-2xl bg-slate-50 p-4">
                  <div className="text-xs font-medium tracking-wide text-slate-400">本轮关注</div>
                  <div className="mt-2 text-sm leading-6 text-slate-700">{item.reviewSummary ?? '当前以开发推进为主，待补充评审重点。'}</div>
                </div>
              </div>
            </section>

            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="mb-4 text-xl font-semibold text-slate-800">开发对位</h2>
              {developmentPlan ? (
                <>
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-2xl bg-slate-50 p-4">
                      <div className="text-xs font-medium tracking-wide text-slate-400">楦型 / 体量方向</div>
                      <div className="mt-2 text-sm text-slate-700">{developmentPlan.silhouette}</div>
                    </div>
                    <div className="rounded-2xl bg-slate-50 p-4">
                      <div className="text-xs font-medium tracking-wide text-slate-400">帮面结构</div>
                      <div className="mt-2 text-sm text-slate-700">{developmentPlan.upperConstruction}</div>
                    </div>
                    <div className="rounded-2xl bg-slate-50 p-4">
                      <div className="text-xs font-medium tracking-wide text-slate-400">大底方向</div>
                      <div className="mt-2 text-sm text-slate-700">{developmentPlan.outsoleDirection}</div>
                    </div>
                    <div className="rounded-2xl bg-slate-50 p-4">
                      <div className="text-xs font-medium tracking-wide text-slate-400">材料 / 配色方向</div>
                      <div className="mt-2 text-sm text-slate-700">{developmentPlan.materialFocus}</div>
                      <div className="mt-1 text-xs text-slate-500">配色：{developmentPlan.colorDirection}</div>
                    </div>
                  </div>
                  <div className="mt-4 rounded-2xl bg-slate-50 p-4">
                    <div className="text-xs font-medium tracking-wide text-slate-400">本轮评审重点</div>
                    <div className="mt-2 text-sm leading-6 text-slate-700">{developmentPlan.reviewFocus}</div>
                  </div>
                </>
              ) : (
                <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">当前还没有建立该单款的开发对位信息。</div>
              )}
            </section>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {reviewConclusion ? (
                  <span className={`rounded-full px-3 py-1 text-xs font-medium ${reviewConclusion.className}`}>{reviewConclusion.label}</span>
                ) : null}
                {reviewRecord ? (
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
                    {stageMap[reviewRecord.stage] ?? reviewRecord.stage}
                  </span>
                ) : null}
              </div>
              ...
              {reviewRecord ? (
                <>
                  <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-700">{reviewRecord.summary}</div>

                  <div className="mt-4 grid gap-4 text-sm text-slate-600 md:grid-cols-4">
                    <div>
                      <div className="text-xs text-slate-400">评审日期</div>
                      <div className="mt-1 font-medium text-slate-900">{formatDate(reviewRecord.review_date)}</div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-400">评审人</div>
                      <div className="mt-1 font-medium text-slate-900">{reviewRecord.reviewer}</div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-400">下次评审</div>
                      <div className="mt-1 font-medium text-slate-900">{formatDate(reviewRecord.next_review_date)}</div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-400">来源文件</div>
                      <div className="mt-1 font-medium text-slate-900">{reviewRecord.source_file}</div>
                    </div>
                  </div>
                  ...
                  {reviewDimensions.length > 0 ? (
                    <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                      {reviewDimensions.map(([key, value]) => {
                        const riskMeta = value?.risk ? RISK_LEVEL_MAP[value.risk] : null;
                        return (
                          <div key={key} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div className="flex items-start justify-between gap-3">
                              <div className="text-sm font-semibold text-slate-900">{dimensionLabels[key] ?? key}</div>
                              {riskMeta ? (
                                <span className={`rounded-full px-2 py-1 text-[11px] font-medium ${riskMeta.bgColor} ${riskMeta.textColor}`}>
                                  {riskMeta.label}
                                </span>
                              ) : null}
                            </div>
                    ...
                    <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">当前评审记录还没有拆分到结构维度。</div>
                  )}
                        ...
                        reviewRecord.change_actions.map((action, index) => (
                          <div key={`${action.type}-${index}`} className="rounded-xl border border-slate-200 bg-white p-3">
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <span className="text-xs text-slate-500">责任人：{action.responsible}</span>
                            </div>
                            <div className="mt-2 text-sm leading-6 text-slate-700">{action.detail}</div>
                          </div>
                        ))
                      ) : (
                        <div className="rounded-xl border border-dashed border-slate-200 bg-white p-3 text-sm text-slate-500">当前评审记录没有拆分出的具体修改动作。</div>
                      )}
                    </div>
                  </div>
                        ...
                        reviewRecord.version_history.map((version) => (
                          <div key={version.version} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <div className="text-xs text-slate-500">{formatDate(version.date)}</div>
                            </div>
                            <div className="mt-2 text-sm leading-6 text-slate-600">{version.summary}</div>
                          </div>
                        ))
                      ) : (
                        <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">当前还没有版本历史记录。</div>
                      )}
                    </div>
                  </div>
                </>
              ) : (
                <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">当前还没有关联到该单款的 OpenClaw 评审记录。</div>
              )}
            </section>

            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-xl font-semibold text-slate-800">修改单</h2>
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">{changeOrders.length} 项</span>
              </div>
              {changeOrders.length > 0 ? (
                    ...
                    <div key={changeOrder.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      <div className="mt-3 grid gap-3 text-sm text-slate-600 md:grid-cols-2">
                        <div>
                          ...
                          {changeOrder.responsible}
                        </div>
                        <div>
                          <span className="text-slate-400">截止时间：</span>
                          {formatDate(changeOrder.deadline)}
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">本轮还没有正式生成修改单。</div>
              )}
            </section>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 grid grid-cols-2 gap-4 text-sm text-slate-600">
                <div>
                  ...
                  <div className="mt-1 text-slate-900">{formatDateTime(item.createdAt)}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400">最后更新</div>
                  <div className="mt-1 text-slate-900">{formatDateTime(item.updatedAt)}</div>
                </div>
              </div>
            </section>
          </div>
